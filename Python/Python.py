...
def findAddress(address):
	...
	while address[0] != stack[tmp + 3]:
		...
		if tmp < 1:
			print("wrong memory access")
			exit(1)
	return tmp + 3 + address[1]	# 그 block번호를 갖는 함수의 offset(address[1])에 해당되는 주소를 리턴
...
while True:
	...
	# ldp
		...
		stack.setSP(stack.sp + 4)	# 해당 함수의 정보를 저장하기 위해서
	# proc
	elif op == 34:
		stack[curntFuncStartP + 3] = operand[1]			# 자신의 블록넘버를 넣기 위해
		stack.setSP(curntFuncStartP + operand[0] + 3)	# 할당 받을 량만큼 sp를 늘려준다.
